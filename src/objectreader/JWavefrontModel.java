                        ...
                        switch (line.charAt(0)) {
                            ...
                            case 'm':                  /*
                                ...
                                if (token.equals("map_Kd")) {
                                    ...
                                    if (texindex < 0) {
                                        file = new File(pathname.getParent() + File.separator + name);
                                        if (file.exists()) {
                                            //BufferedImage image = ImageIO.read(file);
                                            //ImageUtil.flipImageVertically(image); //vertically flip the image
                                            textures[numtextures_aux].name = name;
                                            //textures[numtextures_aux].texturedata = TextureIO.newTexture(image, false);
                                            textures[numtextures_aux].texturedata = TextureIO.newTextureData(GLProfile.getDefault(), file, 
                                                    false, null);
        ...
        while (group != null) {
            if ((mode & WF_TEXTURE) > 0) {
                ...
                if (material.texindex >= 0) {
                    ...
                    com.jogamp.opengl.util.texture.Texture texture = TextureIO.newTexture(texturedata);
